...
//        model.addAttribute("menuList", menuList);
    ...
    /*@RequestMapping(value = "/nextMonthMenu", method = {RequestMethod.POST})
    public String nextMenuList(Model model, @RequestParam Map<String, Object> response) throws Exception{
        ...
        int month = Integer.parseInt(response.get("month").toString());

        if(nowYear == year && month - nowMonth == 1) {
            List<MenuDto> menuList2 = menuService.getList(year, month);

            model.addAttribute("menuList2", menuList2);
        } else {
            model.addAttribute("menuList2", null);
        }

        return "user/nextMonth";
...
//        re.addFlashAttribute("test", "test");

        return "redirect:/applyMenu/pay/complete";
    }

    @GetMapping("/applyMenu/pay/complete")
    public String payComplete(@AuthenticationPrincipal User user, Model model/*, @RequestParam("test") String a*/) {
